...
app.use(cors({ origin(origin, callback) { if (!origin || allowedOrigins.length === 0 || allowedOrigins.includes(origin)) return callback(null, true); return callback(null, false); }, credentials: true, methods: ['GET','POST','PATCH','PUT','DELETE','OPTIONS'], allowedHeaders: ['Content-Type','Authorization','Accept','Stripe-Signature'] }));
...
const apiLimiter = rateLimit({ windowMs: 60_000, max: Number(process.env.PRISM_IP_RATE_LIMIT || 120), standardHeaders: 'draft-7', legacyHeaders: false, handler: (_req, res) => res.status(429).json({ error: 'Muitas solicitações. Aguarde um momento e tente novamente.', code: 'IP_RATE_LIMIT' }) });
app.use('/api', apiLimiter); app.use(trafficGate); app.use(quotaGate);
app.get('/api/health', async (_req, res) => { try { await pool.query('SELECT 1'); res.json({ ok: true, database: 'connected', auth: Boolean(process.env.JWT_SECRET), ai: Boolean(process.env.GEMINI_API_KEY || process.env.GROQ_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY), mcp: Boolean(process.env.MCP_ENCRYPTION_KEY), stripe: Boolean(process.env.STRIPE_SECRET_KEY && process.env.PRISM_STRIPE_ENABLED === 'true') }); } catch { res.status(503).json({ ok: false, database: 'unreachable', auth: Boolean(process.env.JWT_SECRET), ai: Boolean(process.env.GEMINI_API_KEY || process.env.GROQ_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY), mcp: Boolean(process.env.MCP_ENCRYPTION_KEY), stripe: Boolean(process.env.STRIPE_SECRET_KEY && process.env.PRISM_STRIPE_ENABLED === 'true') }); } });
...
app.use((err, _req, res, _next) => { console.error('Prism API error:', err); const status = Number.isInteger(err?.status) && err.status >= 400 && err.status < 600 ? err.status : 500; const message = status >= 500 ? 'Erro interno do servidor.' : (err?.message || 'Solicitação inválida.'); res.status(status).json({ error: message, code: err?.code || 'SERVER_ERROR' }); });
export default app;
