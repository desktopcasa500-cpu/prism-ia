...
router.post('/google', async (req, res, next) => {
  try {
    ...
    const existing = await pool.query('SELECT id, email, name, plan, created_at, password_hash FROM users WHERE email = $1', [email]);
    let user = existing.rows[0];

    if (user) {
      if (user.google_id !== payload.sub) {
        await pool.query('UPDATE users SET google_id = $1, avatar_url = $2 WHERE id = $3', [payload.sub, payload.picture || null, user.id]);
      }
      ...
      const result = await pool.query(
        `INSERT INTO users (email, password_hash, name, google_id, avatar_url)
         VALUES ($1, NULL, $2, $3, $4)
         RETURNING id, email, name, plan, created_at`,
        [email, payload.name || email.split('@')[0], payload.sub, payload.picture || null]
      );
      user = result.rows[0];
    }
    ...
    if (error?.name === 'Error' || error?.message?.toLowerCase().includes('jwt')) {
