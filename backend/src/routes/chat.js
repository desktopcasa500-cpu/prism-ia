...
const surface = (value) => {
  ...
  return SURFACES.has(candidate) ? candidate : 'home';
};
...
const titleFor = (current, text) => current && current !== 'Nova conversa'
  ? current
  : clean(text, 64).replace(/\s+/g, ' ') || 'Nova conversa';
...
async function authorize(userId, model, requestedEffort) {
  ...
  if (!Object.prototype.hasOwnProperty.call(MODEL_REQUIREMENTS, model)) {
    return { ok: false, status: 400, code: 'INVALID_MODEL', error: 'Modelo inválido.' };
  }
  const required = MODEL_REQUIREMENTS[model];
  if (user.rank < required) {
    return { ok: false, status: 403, code: 'PLAN_UPGRADE_REQUIRED', error: 'Este modelo não está disponível no seu plano.', requiredPlan: PLAN_NAMES[required] || 'Pro', model };
  }
  if (!EFFORTS.has(requestedEffort) || !validateThinking(model, requestedEffort)) {
    return { ok: false, status: 400, code: 'INVALID_EFFORT', error: 'Nível de pensamento inválido.' };
  }
  if (requestedEffort === 'ultracode' && !PLAN_FEATURES[user.rank]?.ultracode) {
    return { ok: false, status: 403, code: 'PLAN_UPGRADE_REQUIRED', error: 'Ultracode está disponível apenas no plano Empresarial.', requiredPlan: 'Empresarial', model };
  }
...
async function attachments(ids, userId) {
  ...
  if (invalid) {
    const error = new Error('Um ou mais anexos são inválidos.');
    error.status = 400;
  ...
  const result = await pool.query(
    'SELECT id,name,mime_type,size_bytes,content FROM uploads WHERE user_id=$1 AND id=ANY($2::uuid[]) ORDER BY created_at ASC',
    [userId, requested],
  );

  const found = new Set(result.rows.map((row) => String(row.id)));
  const missing = requested.find((id) => !found.has(id));
  if (missing) {
    const error = new Error('Um ou mais anexos não foram encontrados.');
    error.status = 404;
  ...
  const context = result.rows.map((row) => {
    ...
    if (row.content && TEXT_FILES.has(ext)) {
      return `ANEXO: ${row.name}\n${Buffer.from(row.content).toString('utf8').slice(0, 120_000)}`;
    }
...
async function saveTurn({ session, userId, prompt, model, requestedEffort, result, files, requestId }) {
  ...
  try {
    ...
    const userMessage = await client.query(
      `INSERT INTO messages(session_id,user_id,role,content,effort,model_id,metadata)
       VALUES($1,$2,'user',$3,$4,$5,$6)
       RETURNING id,role,content,effort,model_id,metadata,created_at`,
      [session.id, userId, prompt, requestedEffort, model, meta],
    );
    const assistantMessage = await client.query(
      ...
      [
        session.id,
        userId,
        String(result.text || '').trim() || 'Resposta concluída.',
        requestedEffort,
        tokens,
        providers[0] || null,
        model,
        'Resposta concluída e revisada.',
        meta,
      ],
    );
...
router.get('/usage', async (req, res, next) => {
  try { res.json(await getUsage(req.userId)); } catch (error) { next(error); }
});

router.get('/usage/history', async (req, res, next) => {
  try { res.json(await getDailyUsage(req.userId, req.query?.days)); } catch (error) { next(error); }
});

router.get('/usage/account', async (req, res, next) => {
  try { res.json({ usage: await getUsage(req.userId) }); } catch (error) { next(error); }
});

router.get('/sessions', async (req, res, next) => {
  try {
    const result = await pool.query(
      'SELECT id,title,surface,created_at,updated_at FROM sessions WHERE user_id=$1 AND surface=$2 ORDER BY updated_at DESC,created_at DESC LIMIT 100',
      [req.userId, surface(req.query?.surface)],
    );
...
router.post('/sessions', async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO sessions(user_id,title,surface) VALUES($1,$2,$3) RETURNING id,title,surface,created_at,updated_at',
      [req.userId, rawTitle || 'Nova conversa', surface(req.body?.surface)],
    );
...
router.patch('/sessions/:id', async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE sessions SET title=$1,updated_at=now() WHERE id=$2 AND user_id=$3 RETURNING id,title,surface,created_at,updated_at',
...
router.get('/sessions/:id/messages', async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id,role,content,effort,tokens_used,provider,model_id,thinking_summary,metadata,created_at FROM messages WHERE session_id=$1 AND user_id=$2 ORDER BY created_at ASC LIMIT 500',
...
router.post('/sessions/:id/messages', async (req, res, next) => {
  ...
  if (rawPrompt.length > MAX_MESSAGE) return res.status(413).json({ error: `Mensagem limitada a ${MAX_MESSAGE.toLocaleString('pt-BR')} caracteres.`, code: 'MESSAGE_TOO_LARGE' });
  ...
  if (attachmentIds.length > MAX_ATTACHMENTS) return res.status(400).json({ error: `Você pode anexar até ${MAX_ATTACHMENTS} arquivos por mensagem.`, code: 'TOO_MANY_ATTACHMENTS' });
  ...
  try {
    ...
    if (clientRequestId) {
      const duplicate = await pool.query(
        `SELECT id,role,content,effort,tokens_used,provider,model_id,thinking_summary,metadata,created_at
         FROM messages
         WHERE user_id=$1 AND role='assistant' AND metadata->>'client_request_id'=$2
    ...
    if (!reservation.ok) {
      return res.status(reservation.status || 429).json({
        error: 'O limite de uso foi atingido.',
    ...
    return res.status(201).json({
      ...saved,
      usage,
      consumed: Math.max(0, Number(usage?.percentage || 0) - Number(before?.percentage || 0)),
    });
  } catch (error) {
