...
async function historyBefore(id,userId,before){const result=await pool.query('SELECT role,content FROM messages WHERE session_id=$1 AND user_id=$2 AND created_at < $3 ORDER BY created_at DESC LIMIT $4',[id,userId,before,MAX_HISTORY]);return result.rows.reverse().map((row)=>`${row.role}: ${row.content}`).join('\n');}
...
async function saveTurn({session,userId,prompt,model,effort,result,attachments,requestId,projectId,saveUserMessage=true,regeneratedMessageId=null,editedMessageId=null}){const usage=await getUsage(userId);const meta=JSON.stringify({client_request_id:requestId||undefined,attachments,project_id:projectId||null,requested_effort:effort,providers_used:result?.providers||[],tools_used:result?.tools_used||[],regenerated_from:regeneratedMessageId||undefined});const client=await pool.connect();try{await client.query('BEGIN');let userMessage=null;if(regeneratedMessageId){const target=await client.query(`SELECT id,created_at FROM messages WHERE id=$1 AND session_id=$2 AND user_id=$3 AND role='assistant' FOR UPDATE`,[regeneratedMessageId,session.id,userId]);if(!target.rows.length)throw Object.assign(new Error('Resposta para regenerar não encontrada.'),{status:404,code:'REGENERATE_TARGET_NOT_FOUND'});await client.query('DELETE FROM messages WHERE session_id=$1 AND user_id=$2 AND created_at>=$3',[session.id,userId,target.rows[0].created_at]);}if(editedMessageId){const target=await client.query(`SELECT id,created_at FROM messages WHERE id=$1 AND session_id=$2 AND user_id=$3 AND role='user' FOR UPDATE`,[editedMessageId,session.id,userId]);if(!target.rows.length)throw Object.assign(new Error('Mensagem para edição não encontrada.'),{status:404,code:'EDIT_TARGET_NOT_FOUND'});await client.query('DELETE FROM messages WHERE session_id=$1 AND user_id=$2 AND created_at>$3',[session.id,userId,target.rows[0].created_at]);const updated=await client.query(`UPDATE messages SET content=$1,effort=$2,model_id=$3,metadata=metadata || $4::jsonb WHERE id=$5 AND user_id=$6 RETURNING id,role,content,effort,model_id,metadata,created_at`,[prompt,effort,model,meta,editedMessageId,userId]);userMessage=updated.rows[0]||null;}else if(saveUserMessage){const savedUser=await client.query(`INSERT INTO messages(session_id,user_id,role,content,effort,model_id,metadata) VALUES($1,$2,'user',$3,$4,$5,$6) RETURNING id,role,content,effort,model_id,metadata,created_at`,[session.id,userId,prompt,effort,model,meta]);userMessage=savedUser.rows[0];}const assistant=await client.query(`INSERT INTO messages(session_id,user_id,role,content,effort,tokens_used,provider,model_id,thinking_summary,metadata) VALUES($1,$2,'assistant',$3,$4,$5,$6,$7,$8,$9) RETURNING id,role,content,effort,tokens_used,provider,model_id,thinking_summary,metadata,created_at`,[session.id,userId,String(result.text||'').trim()||'Resposta concluída.',effort,Number(result.tokens||0),result.providers?.[0]||null,model,'Execução concluída com as ferramentas disponíveis.',meta]);const title=session.title==='Nova conversa'?clean(prompt,64).replace(/\s+/g,' ')||'Nova conversa':session.title;await client.query('UPDATE sessions SET title=$1,updated_at=now() WHERE id=$2 AND user_id=$3',[title,session.id,userId]);await client.query('COMMIT');return{userMessage,message:assistant.rows[0],usage,attachments:result.attachments||attachments,tools:result.tools_used||[],regeneratedMessageId,editedMessageId};}catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}}

async function process({req,input,onProgress,signal}){const {session,prompt,model,effort,attachments,projectId,requestId,surface,regenerateMessageId,regenerateBefore,editMessageId,editBefore}=input;const access=await authorize(req.userId,model,effort);if(!access.ok)throw Object.assign(new Error(access.error),access);const reservation=await reserveUsage(req.userId,model);if(!reservation.ok)throw Object.assign(new Error('O limite de uso foi atingido.'),{status:reservation.status||429,code:reservation.code,usage:reservation.usage});try{onProgress?.({type:'quota_reserved',message:'Uso reservado.'});const files=await getAttachments(attachments,req.userId);const contextHistory=(editBefore||regenerateBefore)?await historyBefore(session.id,req.userId,editBefore||regenerateBefore):await history(session.id,req.userId);const preferences=await pool.query('SELECT assistant_instructions FROM users WHERE id=$1',[req.userId]);const instructions=String(preferences.rows[0]?.assistant_instructions||'').trim().slice(0,6000);const preferenceContext=instructions?`INSTRUÇÕES PESSOAIS DO USUÁRIO:\n${instructions}`:'';const context=[preferenceContext,`SUPERFÍCIE ATUAL: ${surface}`,contextHistory,...files.context].filter(Boolean).join('\n\n').slice(-180000);const finalPrompt=prompt||'Analise os arquivos anexados e responda com base neles.';const result=await runOrchestration(finalPrompt,access.effort,{model},context,req.userId,{projectId:projectId||null,onProgress,signal});if(result?.status!=='ok')throw Object.assign(new Error(result?.message||'Serviço de IA temporariamente indisponível.'),{status:503,code:'AI_UNAVAILABLE'});await recordTokens(reservation.reservationId,result.providers?.[0]||null,Number(result.tokens||0));return saveTurn({session,userId:req.userId,prompt:finalPrompt,model,effort:input.effort,result,attachments:files.items,requestId,projectId,saveUserMessage:!regenerateMessageId&&!editMessageId,regeneratedMessageId:regenerateMessageId||null,editedMessageId:editMessageId||null});}catch(error){await releaseUsage(reservation.reservationId).catch(()=>{});throw error;}}

async function prepare(req){const raw=typeof req.body?.content==='string'?req.body.content:'';if(raw.length>MAX_MESSAGE)throw Object.assign(new Error(`Mensagem limitada a ${MAX_MESSAGE.toLocaleString('pt-BR')} caracteres.`),{status:413,code:'MESSAGE_TOO_LARGE'});let prompt=clean(raw);const model=clean(req.body?.model,80)||'prism-mini-1.0';const effort=clean(req.body?.effort,20).toLowerCase()||'medium';let attachments=Array.isArray(req.body?.attachmentIds)?req.body.attachmentIds:[];const projectId=clean(req.body?.projectId,120)||null;const requestId=requestKey(req.body?.clientRequestId);const regenerateMessageId=clean(req.body?.regenerateMessageId,120);const editMessageId=clean(req.body?.editMessageId,120);const requestedSurface=surface(req.body?.surface);let regenerateBefore=null;let editBefore=null;if(!EFFORTS.has(effort))throw Object.assign(new Error('Nível de pensamento inválido.'),{status:400,code:'INVALID_EFFORT'});if(!isUuid(req.params.id))throw Object.assign(new Error('Identificador de sessão inválido.'),{status:400,code:'INVALID_SESSION_ID'});const session=await getSession(req.params.id,req.userId,requestedSurface);if(!session)throw Object.assign(new Error('Conversa não encontrada.'),{status:404,code:'SESSION_NOT_FOUND'});if(regenerateMessageId&&editMessageId)throw Object.assign(new Error('Regeneração e edição não podem ocorrer juntas.'),{status:400,code:'INVALID_MUTATION_MODE'});if(editMessageId){if(!isUuid(editMessageId))throw Object.assign(new Error('Mensagem de edição inválida.'),{status:400,code:'INVALID_EDIT_MESSAGE_ID'});const target=await pool.query(`SELECT id,created_at,role FROM messages WHERE id=$1 AND session_id=$2 AND user_id=$3`,[editMessageId,session.id,req.userId]);if(!target.rows.length||target.rows[0].role!=='user')throw Object.assign(new Error('Mensagem de edição não encontrada.'),{status:404,code:'EDIT_TARGET_NOT_FOUND'});editBefore=target.rows[0].created_at;}if(regenerateMessageId){if(!isUuid(regenerateMessageId))throw Object.assign(new Error('Mensagem de regeneração inválida.'),{status:400,code:'INVALID_REGENERATE_MESSAGE_ID'});const target=await pool.query(`SELECT id,created_at FROM messages WHERE id=$1 AND session_id=$2 AND user_id=$3 AND role='assistant'`,[regenerateMessageId,session.id,req.userId]);if(!target.rows.length)throw Object.assign(new Error('Resposta para regenerar não encontrada.'),{status:404,code:'REGENERATE_TARGET_NOT_FOUND'});const source=await pool.query(`SELECT content,metadata FROM messages WHERE session_id=$1 AND user_id=$2 AND role='user' AND created_at < $3 ORDER BY created_at DESC LIMIT 1`,[session.id,req.userId,target.rows[0].created_at]);if(!source.rows.length)throw Object.assign(new Error('Mensagem original da resposta não encontrada.'),{status:404,code:'REGENERATE_SOURCE_NOT_FOUND'});regenerateBefore=target.rows[0].created_at;prompt=String(source.rows[0].content||'').trim();const metadata=source.rows[0].metadata&&typeof source.rows[0].metadata==='object'?source.rows[0].metadata:{};attachments=Array.isArray(metadata.attachments)?metadata.attachments.map((item)=>item?.id).filter(Boolean):attachments;}if(!prompt&&!attachments.length)throw Object.assign(new Error('Mensagem vazia.'),{status:400,code:'EMPTY_MESSAGE'});return{session,prompt,model,effort,attachments,projectId,requestId,surface:requestedSurface,regenerateMessageId:regenerateMessageId||null,regenerateBefore,editMessageId:editMessageId||null,editBefore};}
...
async function handle(req,res,stream){
  ...
  const result=await process({req,input,onProgress:send,signal:requestAbort.signal});if(stream)send({type:'result',data:result});else res.status(201).json(result);
}catch(error){
