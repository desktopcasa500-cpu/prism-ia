...
const router = Router();
const ALLOWED_EFFORTS = new Set(['medium', 'ultracode']);
...
router.post('/sessions/:id/messages', async (req, res, next) => {
  ...
  try {
    ...
    if (owns.rows[0].title === 'Nova conversa') {
      const title = content.replace(/\s+/g, ' ').slice(0, 64) || 'Nova conversa';
      await pool.query('UPDATE sessions SET title = $1 WHERE id = $2 AND user_id = $3', [title, req.params.id, req.userId]);
    }
