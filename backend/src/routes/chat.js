...
import { validateThinking } from '../services/modelRouter.js';
...
router.get('/sessions/:id/messages', async (req, res, next) => {
  try {
    ...
    const result = await pool.query('SELECT id, role, content, effort, tokens_used, created_at FROM messages WHERE session_id = $1 AND user_id = $2 ORDER BY created_at ASC', [req.params.id, req.userId]);
...
router.post('/sessions/:id/messages', async (req, res, next) => {
  ...
  if (!validateThinking(model, effort)) return res.status(400).json({ error: 'Nível incompatível com o modelo' });

  try {
    const owns = await pool.query('SELECT id, title FROM sessions WHERE id = $1 AND user_id = $2', [req.params.id, req.userId]);
    if (!owns.rows.length) return res.status(404).json({ error: 'Sessão não encontrada' });

    const history = await pool.query('SELECT role, content FROM messages WHERE session_id = $1 AND user_id = $2 ORDER BY created_at DESC LIMIT 20', [req.params.id, req.userId]);
    const context = history.rows.reverse().map(item => `${item.role}: ${item.content}`).join('\n\n');
    ...
    let result;
    try {
      result = await runOrchestration(content, effort, model, context);
    } catch (error) {
    ...
    const saved = await pool.query(`INSERT INTO messages (session_id, user_id, role, content, effort, tokens_used) VALUES ($1, $2, 'assistant', $3, $4, $5) RETURNING id, role, content, effort, tokens_used, created_at`, [req.params.id, req.userId, text, effort, Number.isFinite(result.tokens) ? result.tokens : 0]);

    await pool.query('INSERT INTO usage (user_id, model, effort, tokens) VALUES ($1, $2, $3, $4)', [req.userId, model, effort, Number.isFinite(result.tokens) ? result.tokens : 0]);

    if (!stream) return res.json({ message: saved.rows[0], model, providers_used: Array.isArray(result.providers) ? result.providers : [] });
    ...
    for (let index = 0; index < text.length; index += chunkSize) {
      res.write(`data: ${JSON.stringify({ type: 'delta', text: text.slice(index, index + chunkSize) })}\n\n`);
      await new Promise(resolve => setTimeout(resolve, 8));
    }
    res.write(`data: ${JSON.stringify({ type: 'done', message: saved.rows[0], model, providers_used: result.providers || [] })}\n\n`);
    res.end();
  } catch (error) { next(error); }
});
