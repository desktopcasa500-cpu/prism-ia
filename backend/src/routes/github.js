...
router.get('/status', async (req, res, next) => {
  try {
    const result = await pool.query('SELECT github_user_id, scope, created_at FROM github_connections WHERE user_id=$1', [req.userId]);
    res.json({ configured: isGithubConfigured(), connected: Boolean(result.rows.length), connection: result.rows[0] || null });
...
router.get('/callback', async (req, res) => {
  try {
    ...
    if (!state.userId || !code) return res.status(400).send('OAuth inválido');
