...
router.post('/', async (req, res, next) => {
  try {
    ...
    const data = String(req.body?.dataBase64 || '').trim();
    ...
    if (!MIME_BY_EXT[ext]) return res.status(415).json({ error: 'Tipo de arquivo não suportado.', code: 'FILE_TYPE_UNSUPPORTED' });
    ...
    if (projectId) {
      const owned = await pool.query('SELECT id FROM projects WHERE id=$1 AND user_id=$2', [projectId, req.userId]);
      if (!owned.rows.length) return res.status(404).json({ error: 'Projeto não encontrado.', code: 'PROJECT_NOT_FOUND' });
    }
    const mimeType = String(req.body?.mimeType || MIME_BY_EXT[ext]);
    const textContent = decodeText(normalized, ext);
    ...
    res.status(201).json({ upload: result.rows[0], textContent, isText: textContent !== null });
...
router.post('/analyze', async (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Arquivo não informado' });
  const ext = extension(name);
  if (!MIME_BY_EXT[ext]) return res.status(415).json({ error: 'Tipo de arquivo não suportado' });
