...
router.post('/', async (req, res, next) => {
  try {
    ...
    const mimeType = String(req.body?.mimeType || MIME_BY_EXT[ext]); const textContent = decodeText(normalized, ext);
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${file.name.replace(/"/g, '')}"`); return res.end(file.content);
  } catch (error) { next(error); }
});
