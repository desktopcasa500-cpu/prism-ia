...
const ALLOWED_REDIRECT_ORIGINS = (process.env.FRONTEND_ORIGIN || process.env.APP_URL || '')
  .split(',')
  .map((value) => value.trim().replace(/\/$/, ''))
  .filter(Boolean);
let stripe = null;
function getStripe() { if (!STRIPE_ENABLED || !STRIPE_SECRET_KEY) return null; if (!stripe) stripe = new Stripe(STRIPE_SECRET_KEY); return stripe; }
...
function safeRedirectUrl(value) {
  ...
  try {
    ...
    if (!ALLOWED_REDIRECT_ORIGINS.includes(origin)) return null;
    return url.toString();
  } catch { return null; }
}
...
export async function createCheckoutSession({ plan, customerEmail, successUrl, cancelUrl, userId }) {
  ...
  const safeSuccessUrl = safeRedirectUrl(successUrl);
  const safeCancelUrl = safeRedirectUrl(cancelUrl);
  if (!priceId || !email || !safeSuccessUrl || !safeCancelUrl || !userId) return { ok: false, code: 'INVALID_CHECKOUT', status: 400, error: 'Plano, email, URLs e usuário são obrigatórios e devem usar uma origem oficial.' };
...
async function provisionFromSubscription(subscription, fallbackUserId = null) {
  ...
  await pool.query('UPDATE users SET stripe_customer_id=$2,stripe_subscription_id=$3,stripe_price_id=$4,plan=$5 WHERE id=$1', [result.rows[0].id, customerId || null, subscription.id || null, priceId || null, active && plan ? plan : 'free']);
}
