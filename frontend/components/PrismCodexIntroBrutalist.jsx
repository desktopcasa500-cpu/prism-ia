...
export default function PrismCodexIntroBrutalist({ onComplete, userName = 'você' }) {
  ...
  const sky = ease(range(time, 3200, 7600)); const greeting = ease(range(time, 9800, 12500)); const demo = ease(range(time, 12100, 18200)); const build = ease(range(time, 17500, 23200)); const ending = ease(range(time, 22600, DURATION));
  return <section className="prism-story" aria-label="Apresentação do Prism Codex">
    ...
    <Scene className="story-black" style={{ opacity: time >= 7600 && time <= 9800 ? 1 : 0 }} />
    <Scene className="story-goodmorning" style={{ opacity: greeting }}><LogoMark className="story-good-logo"/><TypeLine text={GREETING} progress={range(time, 10000, 12100)} className="story-good-line"/></Scene>
    <Scene className="story-demo" style={{ opacity: demo, transform: `translateY(${26 - 26 * demo}px)` }}><div className="story-demo-window"><header><div className="story-dots"><i/><i/><i/></div><span>Prism Codex</span><small>{userName}</small></header><div className="story-demo-body"><aside><b>PRISM</b><span>Home</span><span className="active">Codex</span><span>Workspace</span></aside><main><div className="story-selector"><span>Prism Edge 1.0</span><b>⌄</b></div><div className="story-cursor" style={{ left: `${64 + 10 * ease(range(time, 13500, 14300))}%`, top: `${18 + 6 * ease(range(time, 13500, 14300))}%` }}/><div className="story-menu" style={{ opacity: range(time, 14100, 15000), transform: `translateY(${8 - 8 * ease(range(time, 14100, 15000))}px)` }}><span>Prism Edge 1.0</span><strong>Prism Taff 2.0</strong><span>Prism Tex 1.5</span></div><div className="story-user-line"><span>{userName}</span><em>agora</em></div><div className="story-prompt"><TypeLine text={PROMPT} progress={range(time, 15000, 17600)}/></div><button className="story-send">Enviar</button></main></div></div></Scene>
