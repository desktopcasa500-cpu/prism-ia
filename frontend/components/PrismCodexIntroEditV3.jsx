...
export default function PrismCodexIntroEditV3({ onComplete }) {
  ...
  if (!ready && !reduced) return <section className="prism-codex-edit-v3 pce3-loading" aria-label="Carregando apresentação do Prism Codex"><span>PRISM</span></section>;
  ...
  return <section className="prism-codex-edit-v3" aria-label="Introdução cinematográfica do Prism Codex">
    ...
    <div className="pce3-grade" /><div className="pce3-grain" /><div className="pce3-scan" />
    ...
    <div className="pce3-rule pce3-rule-top" /><div className="pce3-progress"><i style={{ width: `${clamp(time / DURATION) * 100}%` }} /></div>
    <button className="pce3-skip" type="button" onClick={complete}>Pular <b>ESC</b></button>

    <div className="pce3-scene pce3-prism-title" style={{ opacity: 1 - titleOut, transform: `scale(${1 + easeOut(titleOut) * .05})` }}><Letters text="PRISM AI" className="pce3-script" /><span>ARTIFICIAL INTELLIGENCE / COMPUTATIONAL SYSTEMS</span></div>
    <div className="pce3-scene pce3-glitch" style={{ opacity: titleOut }} aria-hidden="true"><Letters text="PRISM AI" className="pce3-glitch-copy" /><i className="pce3-slice pce3-slice-a" /><i className="pce3-slice pce3-slice-b" /><i className="pce3-slice pce3-slice-c" /></div>
    ...
    <div className="pce3-scene pce3-editorial" style={{ opacity: editorial, transform: `translate3d(0,${(1 - easeOut(editorial)) * 34}px,0)` }} aria-hidden="true">
      ...
      <div className="pce3-editorial-grid"><div className="pce3-editorial-lead"><small>THE MACHINE</small><strong>INTELLIGENCE<br /><em>IS A MATERIAL.</em></strong><p>MODELS · INFERENCE · AGENTS · COMPUTE</p></div><div className="pce3-editorial-rows">{[['MODELS','INFERENCE / REASONING','01'],['OPENAI','MODELS / RESEARCH','02'],['DATA CENTERS','COMPUTE / SCALE','03'],['AGENTS','TOOLS / EXECUTION','04'],['SOFTWARE','INTERFACE / WORKFLOW','05']].map(([title,detail,number])=><div key={number}><b>{title}</b><span>{detail}</span><em>{number}</em></div>)}</div></div>
      <div className="pce3-editorial-bottom"><span>HUMAN × MACHINE</span><span>{activePhoto.credit}</span></div>
    </div>

    <div className="pce3-scene pce3-columns" style={{ opacity: columns }} aria-hidden="true"><div className="pce3-column pce3-column-top" style={{ transform: `translate3d(0,${-easeInOut(columns) * 125}%,0)` }}><small>SUPERIOR</small><strong>MODELS<br />INFERENCE</strong><span>CONTEXT / REASONING</span><div className="pce3-column-image" style={{ backgroundImage: `url(${PHOTOS[1].src})` }} /></div><div className="pce3-column pce3-column-bottom" style={{ transform: `translate3d(0,${easeInOut(columns) * 125}%,0)` }}><small>INFERIOR</small><strong>AGENTS<br />COMPUTE</strong><span>TOOLS / EXECUTION</span><div className="pce3-column-image" style={{ backgroundImage: `url(${PHOTOS[2].src})` }} /></div></div>

    <div className="pce3-scene pce3-star-scene" style={{ opacity: star }} aria-hidden="true"><div className="pce3-star" style={{ transform: `translate(-50%,-50%) scale(${.1 + easeOut(star) * 1.45})` }} />{Array.from({ length: 24 }, (_, index)=><i key={index} className="pce3-ray" style={{ '--r': `${index * 15}deg`, '--burst': star }} />)}<div className="pce3-burst" style={{ transform: `translate(-50%,-50%) scale(${1 + easeOut(star) * 8})`, opacity: 1 - star }} /></div>
    ...
    <div className="pce3-scene pce3-lockup" aria-hidden="true"><div className="pce3-lockup-ai" style={{ transform: `translate(-50%,${-easeIn(codexDrop) * 160}px)`, opacity: 1 - codexDrop }}>PRISM <b>AI</b></div><div className="pce3-lockup-codex" style={{ transform: `translate(-50%,${(1 - easeOut(codexReturn)) * 65}px)` }}>CODEX</div><div className="pce3-lockup-all" style={{ opacity: allInOne, transform: `translate(-50%,${(1 - easeOut(allInOne)) * 28}px)` }}>ALL IN ONE</div></div>
    <div className="pce3-scene pce3-morph" style={{ opacity: morph }} aria-hidden="true"><div className="pce3-all-word" style={{ opacity: 1 - easeOut(morph), transform: `scale(${1 - easeOut(morph) * .4})` }}>ALL IN ONE</div><div className="pce3-logo-morph" style={{ opacity: between(morph,.2,.9), transform: `scale(${.75 + easeOut(morph) * .25})` }}><span className="pce3-logo-star">✦</span><strong>PRISM</strong></div></div>
    ...
    <div className="pce3-scene pce3-composer-scene" style={{ opacity: composer, transform: `translateY(${(1 - easeOut(composer)) * 22}px) scale(${.98 + composer * .02})` }}><div className="pce3-composer"><header><span>PRISM CODEX</span><b>NEW PROJECT / 001</b></header><div className="pce3-composer-text">{PROMPT.slice(0,promptCount)}<i /></div><footer><span>DESCRIBE WHAT YOU WANT TO BUILD</span><b>Prism Taff 2.0 ↗</b></footer></div></div>
    <div className="pce3-scene pce3-models" style={{ opacity: models, transform: `translateY(${(1 - easeOut(models)) * 18}px)` }}><div className="pce3-model-head">MODELOS</div>{MODELS.map((label,index)=><div key={label} className={`pce3-model-row ${index===MODELS.length-1 && modelSelected ? 'selected' : ''}`}><span>0{index+1}</span><b>{label}</b>{index===MODELS.length-1 && modelSelected?<i>✓</i>:null}</div>)}</div>
    <div className="pce3-scene pce3-agent" style={{ opacity: agent }}><div className="pce3-agent-bg" style={{ backgroundImage: `url(${PHOTOS[0].src})`, transform: `scale(${1.02 + easeOut(agent) * .08})` }} /><div className="pce3-agent-grade" /><div className="pce3-agent-content"><span>PRISM TAFF 2.0 / {agentLive ? 'AGENT ONLINE' : agentState === 'failed' ? 'UNAVAILABLE' : 'STARTING'}</span><strong>ANALYZE <i>→</i> BUILD <i>→</i> REVIEW</strong><small>{agentState === 'failed' ? 'Backend indisponível para esta execução.' : 'workspace · files · preview · real execution path'}</small></div><div className={`pce3-agent-status state-${agentState}`}><i />{agentState === 'failed' ? 'UNAVAILABLE' : agentState === 'completed' ? 'DONE' : agentLive ? 'WORKING' : 'STARTING'}</div></div>
