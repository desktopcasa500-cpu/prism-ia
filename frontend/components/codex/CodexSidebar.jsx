...
export default function CodexSidebar({ sessions = [], activeId, query = '', onQuery, onNew, onOpen, onRename, onDelete, onReplay, onMode, mode = 'chat' }) {
  ...
  return <aside className="pcx-sidebar">
    ...
    <label className="pcx-history-search">
      <span>⌕</span>
      <input value={query} onChange={(event) => onQuery?.(event.target.value)} placeholder="Buscar" aria-label="Buscar conversas" />
      <kbd>⌘K</kbd>
    </label>
    ...
    <div className="pcx-sidebar-foot">
      ...
      <div className="pcx-session-note"><span>SESSÃO</span><strong>Contexto isolado</strong></div>
    </div>
  </aside>;
}
