...
export default function Chat() {
  ...
  const sidebar = (
    <aside className={`chat-sidebar ${sidebarCollapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
      ...
      <div className="sidebar-bottom"><div className="sidebar-usage" title={`${usageLabel} · janela de ${usage?.windowHours || 5} horas`}><div className="sidebar-usage-head"><span className="nav-label">Uso</span><strong>{usageLabel}</strong></div><div className="usage-track" aria-label={usageLabel}><span style={{ width: `${usagePct}%` }} /></div><small className="usage-reset nav-label">{usage?.resetsAt ? `Renova às ${formatTime(usage.resetsAt)}` : 'Janela de 5 horas'}</small></div><button className="profile-button" onClick={() => navigate('/configuracoes')}><span className="avatar">{initial(user?.name)}</span><span className="profile-text nav-label"><strong>{user?.name || 'Usuário'}</strong><small>{user?.plan || 'Grátis'}</small></span></button></div>
    </aside>
  );
