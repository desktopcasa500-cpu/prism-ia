...
export default function Chat() {
  ...
  const refreshUsage = useCallback(async () => {
    try {
      const result = await api.get('/chat/usage');
  ...
  useEffect(() => { const timer = setInterval(refreshUsage, 30_000); return () => clearInterval(timer); }, [refreshUsage]);
  useEffect(() => {
    try {
    ...
    <aside className={`chat-sidebar ${sidebarCollapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
      <div className="sidebar-top">
        <div className="sidebar-brand-row">
          ...
          <button className="sidebar-toggle" onClick={() => setSidebarCollapsed((value) => !value)} aria-label={sidebarCollapsed ? 'Expandir barra lateral' : 'Recolher barra lateral'}>{sidebarCollapsed ? '→' : '←'}</button>
        </div>
              ...
              <div key={session.id} className={`session-item ${session.id === activeSession ? 'active' : ''}`}>
                {editingId === session.id ? (
                  <div className="session-rename" ref={renameRef}>
                    <input value={editingTitle} maxLength={120} autoFocus onChange={(event) => setEditingTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') renameSession(session); if (event.key === 'Escape') setEditingId(null); }} aria-label="Novo nome da conversa" />
      ...
      <div className="sidebar-bottom">
        ...
        <button className="theme-button" onClick={() => setDark((value) => !value)}><span className="theme-glyph">{dark ? 'Sol' : 'Lua'}</span><span className="nav-label">{dark ? 'Modo claro' : 'Modo escuro'}</span></button>
    ...
    <div className={`chat-app chat-theme-${dark ? 'dark' : 'light'} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <main className="chat-main">
        <header className="chat-topbar">
          <div className="chat-context"><span>PRISM IA</span>{activeSession && <small>{sessions.find((item) => item.id === activeSession)?.title || 'Nova conversa'}</small>}</div>
          <div className="model-control" ref={pickerRef}>
            <button className="topbar-model" onClick={() => setPickerMode((current) => current === 'closed' ? 'models' : 'closed')} aria-expanded={pickerMode !== 'closed'}><span>{selectedModel.name}</span><small>{selectedEffort.name}</small><b>⌄</b></button>
            {pickerMode !== 'closed' && (
              <div className="model-picker">
                {pickerMode === 'models' ? (
                  <>
                    <div className="picker-head"><span>MODELO</span><strong>Escolha o modelo</strong></div>
                    <div className="picker-list">{MODELS.map((item) => { const locked = isLocked(item); return <button key={item.id} className={`picker-model ${model === item.id ? 'selected' : ''} ${locked ? 'locked' : ''}`} onClick={() => chooseModel(item.id)} aria-disabled={locked}><span><strong>{item.name}{item.badge && <em>{item.badge}</em>}</strong><small>{item.detail}</small></span><b>{locked ? 'Fazer Upgrade' : model === item.id ? 'Atual' : ''}</b></button>; })}</div>
                    <button className="picker-submenu" onClick={() => setPickerMode('thinking')}><span><strong>Nível de pensamento</strong><small>{selectedEffort.name} · {selectedEffort.note}</small></span><b>→</b></button>
                  </>
                ) : (
                  <>
                    <div className="picker-head back"><button onClick={() => setPickerMode('models')} aria-label="Voltar">←</button><div><span>PENSAMENTO</span><strong>Quanto esforço aplicar</strong></div></div>
        ...
        <section className="messages" aria-live="polite">
          {loadingMessages ? <div className="message-loading"><span /><span /><span /></div> : !messages.length ? (
            <div className="empty-chat"><span className="empty-kicker">PRISM IA</span><h1>Olá, {first}.</h1><p>O que vamos fazer hoje?</p><div className="prompt-suggestions">
        ...
        <div className="composer-area">
          ...
          <div className="composer">
            <textarea ref={textareaRef} rows={1} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="Escreva uma mensagem" disabled={sending} aria-label="Mensagem" />
            <div className="composer-footer"><button className="composer-model" onClick={() => setPickerMode('models')}>{selectedModel.short}<span>·</span>{selectedEffort.name}</button><button className="send-button" onClick={send} disabled={!input.trim() || sending}>{sending ? 'Enviando' : 'Enviar'}</button></div>
          </div>
