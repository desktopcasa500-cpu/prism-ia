...
export default function ChatRelease() {
  ...
  async function send() {
    if (!canSend) return;
    setSending(true); setError(''); setEvents([]); setCommandOutput(''); setArtifact(null); setTraceOpen(true);
    ...
    try {
      ...
      setSending(false); controllerRef.current = null;
  ...
  return <div className="prism-agent-chat">
    <aside className="prism-agent-sidebar">
      ...
      <div className="prism-agent-sidebar__nav">
        <button className="active"><PrismIcon name="home" size={15}/>Início</button>
        <button onClick={() => setSearch(search ? '' : ' ')}><PrismIcon name="search" size={15}/>Conversas</button>
        <button onClick={() => navigate('/studio')}><PrismIcon name="folder" size={15}/>Projetos</button>
        <button onClick={() => navigate('/modelos')}><PrismIcon name="model" size={15}/>Modelos</button>
        <button onClick={() => navigate('/configuracoes')}><PrismIcon name="settings" size={15}/>Configurações</button>
      </div>
      <div className="prism-agent-sidebar__projects">
        ...
        <input value={search.trim() === ' ' ? '' : search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar" style={{width:'calc(100% - 18px)',margin:'0 9px 6px',height:30,border:'1px solid #e0dfd9',borderRadius:7,padding:'0 8px',background:'#fff',fontSize:10,outline:0}} />
    ...
    <main className="prism-agent-main">
      <header className="prism-agent-topbar">
        ...
        <div className="prism-agent-topbar-right">
          ...
          <div style={{position:'relative'}}><button className="prism-agent-model-select" onClick={() => setModelOpen((value) => !value)}>{selected.label}⌄</button>{modelOpen && <div style={{position:'absolute',right:0,top:38,zIndex:20,width:260,background:'#fff',border:'1px solid #dddcd5',borderRadius:10,padding:6,boxShadow:'0 18px 40px rgba(20,20,18,.12)'}}>{MODELS.map((item) => <button key={item.id} onClick={() => chooseModel(item.id)} style={{width:'100%',height:38,border:0,borderRadius:7,background:item.id===model?'#f0f0ec':'transparent',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'0 9px',fontSize:10,color:'#37362f'}}><span>{item.label}</span><small style={{color:'#9b9a93'}}>{rank < item.rank ? 'Upgrade' : item.id===model ? 'Selecionado' : ''}</small></button>)}</div>}</div>
        </div>
      </header>
      ...
      <section className="prism-agent-main-scroll">
        <div className="prism-agent-content">
          ...
          {error && <div style={{padding:'10px 0',color:'#9c4a45',fontSize:11}}>{error}</div>}
