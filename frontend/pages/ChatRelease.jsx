...
export default function ChatRelease() {
  ...
  async function newSession() { if (sending) return; try { const result = await api.post('/chat/sessions', { surface: 'home' }); setSessions((list) => [result.session, ...list]); setActiveSession(result.session.id); setMessages([]); setInput(''); setAttachments([]); setAttachmentsUploading(false); setArtifacts([]); setActiveArtifact(null); setArtifactOpen(false); } catch (error) { if (!authFail(error)) setError(error.message || 'Não foi possível criar uma conversa.'); } }
  async function send() { const content = input.trim(); if (!content || sending || attachmentsUploading || lockRef.current) return; lockRef.current = true; setSending(true); setError(''); const abort = new AbortController(); setController(abort); let sessionId = activeSession; const rid = requestId(); const localId = `local-${rid}`; const selectedAttachments = [...attachments]; const attachmentIds = selectedAttachments.map((item) => item.id).filter(Boolean); try { if (!sessionId) { const created = await api.post('/chat/sessions', { surface: 'home', title: content.slice(0, 64) }); sessionId = created.session.id; setActiveSession(sessionId); setSessions((list) => [created.session, ...list]); } const optimistic = { id: localId, role: 'user', content, model_id: model, effort, metadata: { local: true, attachments: selectedAttachments }, created_at: new Date().toISOString() }; setMessages((list) => [...list, optimistic]); setInput(''); const response = await api.post(`/chat/sessions/${encodeURIComponent(sessionId)}/messages`, { content, model, effort, clientRequestId: rid, attachmentIds }, { timeout: 180000, signal: abort.signal }); if (response.duplicate) { const history = await api.get(`/chat/sessions/${encodeURIComponent(sessionId)}/messages?surface=home`); setMessages(history.messages || []); } else if (response.message) { setMessages((list) => [...list.filter((item) => item.id !== localId), response.userMessage || optimistic, response.message]); } setAttachments([]); if (response.usage) setUsage(response.usage); } catch (error) { if (error?.name !== 'AbortError') { if (error?.payload?.code === 'MODEL_QUOTA_REQUIRED') { setError('Adicione créditos para usar este modelo.'); setRequestedModel(selected.label); setQuotaBlocked(true); setPlansOpen(true); } else if (error?.status === 403 || error?.status === 429) { setRequestedModel(error.payload?.requiredPlan ? `Plano ${error.payload.requiredPlan}` : selected.label); setQuotaBlocked(false); setPlansOpen(true); } else if (!authFail(error)) setError(error.message || 'Não foi possível concluir a resposta.'); } setMessages((list) => list.filter((item) => item.id !== localId)); } finally { setSending(false); setController(null); lockRef.current = false; setAttachmentsUploading(false); refreshUsage(); requestAnimationFrame(() => textareaRef.current?.focus()); } }
