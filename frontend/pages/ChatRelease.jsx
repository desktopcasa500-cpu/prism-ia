...
export default function ChatRelease() {
  ...
  const selected = useMemo(() => MODELS.find((item) => item.id === model) || MODELS[1], [model]); const canSend = Boolean(input.trim()) && !sending && !attachmentsUploading;
  ...
  async function send() {
    ...
    try {
      ...
      const optimistic = { id: localId, role: 'user', content, model_id: model, effort, metadata: { local: true, attachment_ids: attachmentIds }, created_at: new Date().toISOString() }; setMessages((list) => [...list, optimistic]); setInput('');
