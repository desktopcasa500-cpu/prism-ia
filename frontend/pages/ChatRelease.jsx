    ...
    <main className="prism-agent-main">
      ...
      <header className="prism-agent-topbar">
        ...
        <div className="prism-agent-topbar-right">
          ...
          <div className="prism-agent-model-picker-wrap"><button className="prism-agent-model-select" onClick={() => setModelOpen((value) => !value)} aria-haspopup="menu" aria-expanded={modelOpen}>{selected.label}<span aria-hidden="true">⌄</span></button>{modelOpen && <div className="prism-agent-model-picker" role="menu">{MODELS.map((item) => <button type="button" role="menuitem" aria-current={item.id === model ? "true" : undefined} key={item.id} onClick={() => chooseModel(item.id)}><span>{item.label}</span><small>{rank < item.rank ? 'Upgrade' : item.id===model ? 'Selecionado' : ''}</small></button>)}</div>}</div><button className="prism-agent-theme-toggle" type="button" onClick={cycleMode} aria-label={`Tema: ${themeMode}`} title={`Tema: ${themeMode === "system" ? "sistema" : themeMode === "dark" ? "escuro" : "claro"}`}><PrismIcon name={theme === "dark" ? "moon" : "sun"} size={14}/></button>
        </div>
      </header>
