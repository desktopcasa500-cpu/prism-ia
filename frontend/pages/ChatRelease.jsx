...
export default function ChatRelease() {
  ...
  useEffect(() => { const found = []; for (const message of messages) if (message.role === 'assistant') found.push(...extractCodeBlocks(message.content, String(message.id))); setArtifacts((current) => { const map = new Map(current.map((item) => [item.id, item])); for (const item of found) map.set(item.id, item); return [...map.values()]; }); }, [messages]);
  async function newSession() { if (sending) return; try { const result = await api.post('/chat/sessions', { surface: 'home' }); setSessions((list) => [result.session, ...list]); setActiveSession(result.session.id); setMessages([]); setInput(''); setArtifacts([]); } catch (e) { if (!authFail(e)) setError(e.message || 'Não foi possível criar uma conversa.'); } }
  async function send() {
  ...
  return <div className={`prism-release-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="prism-chat-main">
      ...
      {artifactOpen && <aside style={{ position: 'fixed', top: 56, right: 0, bottom: 0, width: 'min(430px, 92vw)', background: '#100e0c', borderLeft: '1px solid var(--prism-border)', zIndex: 60, display: 'grid', gridTemplateRows: 'auto minmax(0,1fr)' }}><div style={{ padding: 12, borderBottom: '1px solid var(--prism-border)', display: 'flex', justifyContent: 'space-between' }}><strong>Artefatos</strong><button onClick={() => setArtifactOpen(false)}>Fechar</button></div><Artifacts items={artifacts} selectedId={activeArtifact} onSelect={setActiveArtifact} /></aside>}
