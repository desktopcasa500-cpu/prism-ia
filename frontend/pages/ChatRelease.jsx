...
export default function ChatRelease() {
  ...
  async function send() {
    const content = input.trim(); if (!content || sending || attachmentsUploading || lockRef.current) return; lockRef.current = true; setSending(true); setError(''); const abort = new AbortController(); setController(abort); let sessionId = activeSession; const rid = requestId(); const localId = `local-${rid}`; const attachmentIds = attachments.map((item) => item.id).filter(Boolean);
    try {
      ...
      const optimistic = { id: localId, role: 'user', content, model_id: model, effort, metadata: { local: true, attachment_ids: attachmentIds, attachments }, created_at: new Date().toISOString() }; setMessages((list) => [...list, optimistic]); setInput('');
      ...
      if (response.duplicate) { const history = await api.get(`/chat/sessions/${encodeURIComponent(sessionId)}/messages?surface=home`); setMessages(history.messages || []); setAttachments([]); }
      else if (response.message) { setMessages((list) => [...list.filter((item) => item.id !== localId), response.userMessage || optimistic, response.message]); setAttachments([]); }
