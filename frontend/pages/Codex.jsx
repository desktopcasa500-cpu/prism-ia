...
function fileArtifact(file) {
  ...
  const language = {
    html: 'markup', htm: 'markup', css: 'css', js: 'javascript', jsx: 'jsx', ts: 'typescript',
    tsx: 'tsx', json: 'json', md: 'markdown', py: 'python',
  }[extension] || 'text';
  return {
    id: `workspace:${file.path}`,
    filename: file.path,
    language,
    languageLabel: extension.toUpperCase(),
  ...
  return (
    <div className="codex-steps">
      {PHASES.map(([id, label], index) => (
        <div key={id} className={`codex-step ${index < active ? 'done' : ''} ${id === phase ? 'active' : ''}`}>
          <span>{index < active ? '✓' : id === phase ? '·' : ''}</span>
          <div><strong>{label}</strong><small>{steps.find((item) => item.phase === id)?.detail || ''}</small></div>
        </div>
      ))}
    </div>
  );
...
export default function Codex() {
  ...
  const loadProject = useCallback(async () => {
    ...
    if (!project) {
      ...
      await Promise.all(STARTER.map((file) => api.post('/files', {
    ...
    const loadedArtifacts = loaded.filter((file) => file.kind !== 'folder').map(fileArtifact);
    setProjectId(project.id);
    setProjectName(project.name || 'Novo projeto');
    setFiles(loaded);
    setArtifacts((current) => mergeArtifacts(current, loadedArtifacts));
  }, []);
  ...
  useEffect(() => {
    Promise.all([
      loadProject(),
      loadSessions(),
      api.get('/mcp').then((data) => setMcpServers(data.servers || [])).catch(() => setMcpServers([])),
    ]).catch((cause) => setError(cause.message || 'Não foi possível carregar o Codex.'));
  ...
  async function renameSession(session) {
    ...
    const data = await api.patch(`/chat/sessions/${encodeURIComponent(session.id)}`, { title: title.trim() });
  ...
  async function sendChat(value) {
    ...
    try {
      if (!sid) {
        ...
        setSessionId(sid);
        setSessions((current) => [created.session, ...current.filter((item) => item.id !== sid)]);
      }
      ...
      setRunning(true);
      setError('');
      const result = await api.post(`/chat/sessions/${encodeURIComponent(sid)}/messages`, {
        content: value,
        model,
        effort,
      }, { timeout: 180000 });
      ...
      const answer = mapMessage(result.message);
      const next = [...base, answer];
      setMessages(next);
      cacheSession(sid, next).catch(() => {});
      ...
      setRunning(false);
      window.setTimeout(() => { sendLockRef.current = false; }, 150);
    }
  }
    ...
    try {
      const result = await api.streamPost('/ai/generate/stream', {
        ...
        if (event.type === 'phase') {
          ...
          setSteps((current) => [...current.filter((item) => item.phase !== event.phase), event]);
        }
      ...
      setMessages((current) => [...current, {
        ...
        role: 'assistant',
        text: result.text || 'Projeto atualizado.',
        tools: [],
      }]);
      setPhase('completed');
    } catch (cause) {
  ...
  async function send() {
    ...
    if (mode === 'vibe' || VIBE_PATTERN.test(value)) await sendVibe(value);
    else await sendChat(value);
  }
  ...
  return (
    <div className={`codex-rebuild ${mode === 'vibe' ? 'vibe-mode' : 'chat-mode'} ${artifactPanelOpen ? 'artifact-open' : ''}`}>
      ...
      <main className="codex-main-rebuild">
        <header className="codex-header-rebuild">
          ...
          <div className="codex-header-actions">
            <div className="codex-model-wrap">
              <button type="button" onClick={() => setModelOpen((current) => !current)}>{activeModelLabel}<span>⌄</span></button>
              {modelOpen && <div className="codex-model-popover">
                {MODELS.map(([id, label]) => <button type="button" key={id} className={id === model ? 'active' : ''} onClick={() => { setModel(id); setModelOpen(false); }}>{label}{id === model && <small>Atual</small>}</button>)}
              </div>}
            </div>
            <button type="button" onClick={() => setEffort(EFFORTS[(EFFORTS.indexOf(effort) + 1) % EFFORTS.length])}>{effort}</button>
            <button type="button" onClick={() => setCommandOpen(true)}>⌘K</button>
          </div>
        </header>
        ...
        <McpContextBar
          servers={mcpServers}
          activeIds={mcpActive}
          onToggle={(id) => setMcpActive((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id])}
        />

        <section className="codex-body-rebuild">
          <section className="codex-conversation-rebuild">
            <div className="codex-scroll-rebuild">
              {!messages.length && <div className="codex-welcome-rebuild">
                <span>PRISM CODEX</span>
                <h1>Construa sem sair do fluxo.</h1>
                <p>Uma conversa limpa para planejar, escrever e revisar. Quando houver código, ele abre na superfície dedicada.</p>
                <div>
                  <button type="button" onClick={() => setPrompt('Crie um site institucional moderno em HTML, CSS e JavaScript.')}>Criar um site</button>
                  <button type="button" onClick={() => setPrompt('Revise meu projeto e aponte os problemas mais importantes.')}>Revisar projeto</button>
                </div>
              </div>}

              {messages.map((message) => <article className={`codex-message-rebuild ${message.role}`} key={message.id}>
                <div className="codex-message-meta"><span>{message.role === 'user' ? (user?.name || 'Você') : 'Prism IA'}</span></div>
                <div className="codex-message-content">
                  {message.role === 'assistant'
                    ? <MarkdownMessage
                        ...
                        onOpenCode={(id, block) => {
                          setArtifacts((current) => mergeArtifacts(current, [block]));
                ...
                {message.tools?.length > 0 && <small className="codex-tools">Ferramentas · {message.tools.map((item) => item.tool).filter(Boolean).join(', ')}</small>}
              </article>)}

              {running && <div className="codex-running-rebuild"><span />{mode === 'vibe' ? 'Trabalhando no projeto…' : 'Prism está pensando…'}</div>}
              {mode === 'vibe' && phase && <div className="codex-progress-rebuild">
                <header><strong>{PHASES.find(([id]) => id === phase)?.[1] || 'Trabalhando'}</strong><span>{(elapsed / 1000).toFixed(1)}s</span></header>
                <StepList phase={phase} steps={steps} />
              </div>}
            </div>

            {error && <div className="codex-error-rebuild"><span>{error}</span><button type="button" onClick={() => setError('')}>Fechar</button></div>}

            <footer className="codex-composer-rebuild">
              <div className="codex-composer-box">
                <textarea
                  ...
                  disabled={running}
                  rows={1}
                  placeholder={mode === 'vibe' ? 'Descreva o que você quer construir…' : 'Escreva uma mensagem'}
                />
                <div>
                  <span>{mode === 'vibe' ? 'Vibe Code' : 'Enter envia · Shift + Enter quebra a linha'}</span>
                  {running ? <button type="button" onClick={stop}>Parar</button> : <button type="button" className={canSend ? 'ready' : ''} onClick={send} disabled={!canSend}>Enviar</button>}
                </div>
              </div>
              <small>Revise informações importantes antes de usá-las.</small>
            </footer>
          </section>

          {mode === 'vibe' && <section className="codex-live-workspace">
            <header>
              <div><span>PROJETO</span><strong>{projectName}</strong></div>
              <button type="button" onClick={sidebarArtifacts}>Abrir arquivos</button>
            </header>
            <div>{previewFor(files)
              ? <iframe title="Preview do projeto" srcDoc={previewFor(files)} sandbox="allow-scripts" />
              : <div className="codex-no-preview">Gere um arquivo HTML para abrir o preview.</div>}
            </div>
          </section>}
        </section>
      </main>
      ...
      <CodeArtifactsPanel
        open={artifactPanelOpen && artifacts.length > 0}
        artifacts={artifacts}
        activeId={activeArtifactId}
        onSelect={setActiveArtifactId}
        onClose={() => setArtifactPanelOpen(false)}
        onUpdateArtifact={(id, patch) => setArtifacts((current) => current.map((item) => item.id === id ? { ...item, ...patch } : item))}
      />

      {plansOpen && <div className="codex-modal-rebuild" onMouseDown={() => setPlansOpen(false)}>
        <section onMouseDown={(event) => event.stopPropagation()}>
          <header><strong>Planos</strong><button type="button" onClick={() => setPlansOpen(false)}>Fechar</button></header>
          <p>Escolha o plano que melhor atende ao seu uso.</p>
        </section>
      </div>}

      {commandOpen && <div className="codex-modal-rebuild" onMouseDown={() => setCommandOpen(false)}>
        <section className="codex-command-rebuild" onMouseDown={(event) => event.stopPropagation()}>
          <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar conversas…" />
          <button type="button" onClick={newSession}>+ Nova conversa</button>
          <button type="button" onClick={replayIntro}>Reabrir apresentação</button>
        </section>
      </div>}
    </div>
  );
