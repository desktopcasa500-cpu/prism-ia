...
const PROVIDERS = [
  { id: 'anthropic', label: 'Claude', model: 'claude-3-7-sonnet-latest', enabled: true },
...
function safeReadProviders() {
  try {
    const stored = JSON.parse(localStorage.getItem('prism_codex_providers') || 'null');
    return Array.isArray(stored) && stored.length === 3
      ? PROVIDERS.map((fallback) => ({ ...fallback, ...(stored.find((item) => item.id === fallback.id) || {}) }))
      : PROVIDERS;
...
export default function Codex() {
  ...
  const [models, setModels] = useState(safeReadProviders);
  ...
  useEffect(() => {
    const onKey = (event) => {
      ...
      if (event.key === 'Escape') { setPaletteOpen(false); setModelMenuOpen(false); }
    };
  ...
  function toggleProvider(id) {
    setModels((current) => current.map((item) => item.id === id ? { ...item, enabled: item.enabled === false } : item));
  }
  ...
  async function send() {
    ...
    const selected = models.filter((item) => item.enabled !== false).map(({ id, model }) => ({ provider: id, model }));
    if (!selected.length) { setError('Ative pelo menos um modelo.'); return; }
    setError('');
    let sid = sessionId;
    if (!sid) {
    ...
    setPrompt('');
    setRunning(true);
    setRuns(Object.fromEntries(selected.map((item) => [item.provider, { provider: item.provider, model: item.model, status: 'running', text: '', thinking_summary: '', elapsed_ms: 0 }] )));
  ...
  function introDone() {
    localStorage.setItem('prism_codex_intro_seen', '1');
    setShowIntro(false);
  }
  ...
  return (
    <div className="pcx-root">
      {showIntro && <PrismCodexIntro onComplete={introDone} />}
      <CodexSidebar
        sessions={sessions}
        activeId={sessionId}
        query={query}
      ...
      <main className="pcx-main">
        <header className="pcx-header">
          <div className="pcx-header-title"><span>PRISM CODEX</span><strong>Chat</strong></div>
          <div className="pcx-header-controls">
            <div className="pcx-model-menu">
              <button className="pcx-model-trigger" onClick={() => setModelMenuOpen((open) => !open)} aria-expanded={modelMenuOpen}>
                <span>Modelos</span><b>{models.filter((item) => item.enabled !== false).length}/3</b><i>⌄</i>
              </button>
              {modelMenuOpen && <div className="pcx-model-dropdown">
                {models.map((item) => <button key={item.id} className={item.enabled !== false ? 'on' : ''} onClick={() => toggleProvider(item.id)}><span>{item.label}</span><small>{item.model}</small><b>{item.enabled !== false ? 'ON' : 'OFF'}</b></button>)}
              </div>}
            </div>
            <button className="pcx-effort" onClick={() => setEffort(EFFORTS[(EFFORTS.indexOf(effort) + 1) % EFFORTS.length])}>Pensamento · {effort}</button>
            <button className="pcx-command" onClick={() => setPaletteOpen(true)}>⌘K</button>
          </div>
        </header>
        ...
        <section className="pcx-content">
          <section className="pcx-chat-panel">
            <div className="pcx-chat-scroll">
              {messages.length === 0 && Object.keys(runs).length === 0 && <div className="pcx-welcome"><div className="pcx-welcome-mark">P</div><p>PRISM CODEX</p><h1>O que você quer fazer?</h1><span>Converse normalmente. Uma mesma pergunta pode ser lida por Claude, OpenAI e Gemini sem mudar de conversa.</span></div>}
              {messages.map((message) => <article className={`pcx-message ${message.role}`} key={message.id}><div className="pcx-message-label">{message.role === 'user' ? 'Você' : (message.provider || 'Prism')}</div><div className="pcx-message-text">{message.text}</div>{message.tools.length > 0 && <div className="pcx-message-meta">MCP · {message.tools.map((tool) => tool.tool).filter(Boolean).join(', ')}</div>}</article>)}
              {Object.keys(runs).length > 0 && <ParallelResponseGrid models={models} runs={runs} />}
            </div>
            ...
            <footer className="pcx-composer-wrap">
              <div className="pcx-composer"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="Como posso ajudar você hoje?" rows={1} disabled={running}/><div className="pcx-composer-bar"><span>Enter envia · Shift + Enter quebra</span><div><button className="pcx-replay" onClick={replayIntro}>Apresentação</button><button className="pcx-send" onClick={send} disabled={running || !prompt.trim()}>{running ? 'Processando…' : 'Enviar'}</button></div></div></div>
              <small className="pcx-composer-note">As respostas podem conter erros. Revise informações importantes.</small>
            </footer>
          </section>
        </section>
      </main>
      {paletteOpen && <div className="pcx-modal-backdrop" onMouseDown={() => setPaletteOpen(false)}><div className="pcx-command-palette" onMouseDown={(event) => event.stopPropagation()}><div><span>COMMAND</span><kbd>ESC</kbd></div><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar conversas…"/><button onClick={newSession}>+ Nova conversa</button><button onClick={replayIntro}>Reabrir apresentação</button></div></div>}
    </div>
  );
}
