...
export default function Codex() {
  ...
  const loadProject = useCallback(async () => {
    ...
    setProjectId(project.id); setProjectName(project.name || 'Novo projeto'); setFiles(loaded); setActiveFile(first.path); setCodeSafely(first.content || '');
  }, []);

  function setCodeSafely(value) {
    setCode(value);
  }
  ...
  useEffect(() => {
    const onKey = (event) => {
      ...
      if (event.key === 'Escape') { setCommandOpen(false); setModelOpen(false); }
    };
  ...
  const visibleFiles = useMemo(() => files.filter((file) => !query || file.path.toLowerCase().includes(query.toLowerCase())), [files, query]);

  async function newSession() {
    if (running) return;
    const data = await api.post('/chat/sessions', { title: 'Nova conversa' });
    setSessions((current) => [data.session, ...current]); setSessionId(data.session.id); setMessages([]); setRuns({}); setMode('chat'); setPrompt(''); setCommandOpen(false);
  }
  ...
  async function sendChat(value) {
    let sid = sessionId;
    if (!sid) {
      const created = await api.post('/chat/sessions', { title: value.slice(0, 64) }); sid = created.session.id; setSessionId(sid); setSessions((current) => [created.session, ...current]);
    }
    const userMessage = { id: `local-${Date.now()}`, role: 'user', text: value, tools: [] };
    const nextMessages = [...messages, userMessage];
    setMessages(nextMessages); setPrompt(''); setRunning(true); setError('');
    try {
      if (compareMode) {
  ...
  async function sendVibe(value) {
    ...
    const userMessage = { id: `local-${Date.now()}`, role: 'user', text: value, tools: [] };
    setMessages((current) => [...current, userMessage]); setPrompt(''); setMode('vibe'); setRunning(true); setError(''); setPhase('received'); setSteps([]); startedAt.current = Date.now(); setElapsed(0);
    ...
    try {
      const result = await api.streamPost('/ai/generate/stream', { model, thinking: 'ultracode', prompt: value, context, projectId }, (event) => {
        ...
        if (event.type === 'artifact') {
          setTab('code'); setActiveFile(event.path);
  ...
  async function send() {
    const value = prompt.trim(); if (!value || running) return;
    if (mode === 'vibe' || VIBE_PATTERN.test(value)) { await sendVibe(value); return; }
  ...
  return <div className={`pcx-root pcx-mode-${mode}`}>
    ...
    <CodexSidebar sessions={sessions} activeId={sessionId} query={query} onQuery={setQuery} onNew={newSession} onOpen={loadSession} onRename={renameSession} onDelete={deleteSession} onReplay={replayIntro} onMode={setCodexMode} mode={mode} />

    <main className="pcx-main">
      <header className="pcx-header">
        <div className="pcx-header-title"><span>PRISM</span><strong>{mode === 'vibe' ? 'Vibe Code' : 'Conversa'}</strong></div>
        <div className="pcx-header-controls">
          {mode === 'vibe' && <button className="pcx-view-button" onClick={() => setCodexMode('chat')}>Voltar</button>}
          <div className="pcx-model-menu">
            <button className="pcx-model-trigger" onClick={() => setModelOpen((open) => !open)} aria-expanded={modelOpen}><span>{activeModelLabel}</span><i>⌄</i></button>
            {modelOpen && <div className="pcx-model-dropdown">
              {PRISM_MODELS.map(([id, label]) => <button key={id} className={model === id ? 'on' : ''} onClick={() => { setModel(id); setModelOpen(false); }}><span>{label}</span><b>{model === id ? 'ATIVO' : ''}</b></button>)}
              <div className="pcx-model-divider" />
              <button className={compareMode ? 'on' : ''} onClick={() => { setCompareMode((value) => !value); setModelOpen(false); }}><span>Comparar modelos</span><b>{compareMode ? '3 ATIVOS' : 'OFF'}</b></button>
              {compareMode && providers.map((item) => <button key={item.id} className={`pcx-provider-row ${item.enabled !== false ? 'on' : ''}`} onClick={() => toggleProvider(item.id)}><span>{item.label}</span><small>{item.model}</small><b>{item.enabled !== false ? 'ON' : 'OFF'}</b></button>)}
            </div>}
          </div>
          <button className="pcx-effort" onClick={() => setEffort(EFFORTS[(EFFORTS.indexOf(effort) + 1) % EFFORTS.length])}>{effort}</button>
      ...
      <section className={`pcx-content ${mode === 'vibe' ? 'with-workspace' : ''}`}>
        <section className="pcx-chat-panel">
          <div className="pcx-chat-scroll">
            {messages.length === 0 && !Object.keys(runs).length && mode === 'chat' && <div className="pcx-welcome"><div className="pcx-welcome-mark">P</div><p>PRISM CODEX</p><h1>O que você quer fazer?</h1><span>Comece com uma pergunta, uma ideia ou um problema.</span></div>}
            {messages.map((message) => <article className={`pcx-message ${message.role}`} key={message.id}><div className="pcx-message-label">{message.role === 'user' ? 'Você' : (message.provider || 'Prism')}</div><div className="pcx-message-text">{message.text}</div>{message.files?.length > 0 && <div className="pcx-message-files">{message.files.map((file) => <button key={file} onClick={() => selectFile(files.find((item) => item.path === file))}>{file}</button>)}</div>}{message.tools?.length > 0 && <div className="pcx-message-meta">Ferramentas · {message.tools.map((item) => item.tool).filter(Boolean).join(', ')}</div>}</article>)}
            {Object.keys(runs).length > 0 && <ParallelResponseGrid models={providers} runs={runs} />}
            {mode === 'vibe' && running && <article className="pcx-live-message"><div className="pcx-live-head"><strong>{PHASES.find(([id]) => id === phase)?.[1] || 'Trabalhando'}</strong><span>{(elapsed / 1000).toFixed(1)}s</span></div><StepList phase={phase} steps={steps}/></article>}
          </div>
          ...
          <footer className="pcx-composer-wrap">
            <div className="pcx-composer">
              <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder={mode === 'vibe' ? 'Descreva o que você quer construir…' : 'Como posso ajudar você hoje?'} rows={1} disabled={running}/>
              <div className="pcx-composer-bar"><span>{running ? (mode === 'vibe' ? 'Executando no workspace…' : 'Aguardando respostas…') : 'Enter envia · Shift + Enter quebra a linha'}</span><div><button className="pcx-replay" onClick={replayIntro}>Apresentação</button>{running ? <button className="pcx-stop" onClick={stop}>Parar</button> : <button className="pcx-send" onClick={send} disabled={!prompt.trim()}>Enviar</button>}</div></div>
            </div>
            <small className="pcx-composer-note">As respostas podem conter erros. Revise informações importantes.</small>
          </footer>
        </section>

        {mode === 'vibe' && <section className="pcx-workspace">
          <div className="pcx-workspace-top"><div><span>PROJETO</span><strong>{projectName}</strong></div><nav><button className={tab === 'preview' ? 'active' : ''} onClick={() => setTab('preview')}>Preview</button><button className={tab === 'code' ? 'active' : ''} onClick={() => setTab('code')}>Código</button></nav></div>
          {tab === 'preview' ? <iframe className="pcx-preview" title="Preview do projeto" srcDoc={previewFor(files)} sandbox="allow-scripts" /> : <div className="pcx-editor"><aside className="pcx-tree"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filtrar arquivos" />{visibleFiles.map((file) => <button className={file.path === activeFile ? 'active' : ''} key={file.path} onClick={() => selectFile(file)}>{file.path}</button>)}</aside><main className="pcx-code"><div className="pcx-code-head"><span>{activeFile}</span><small>{code.length} caracteres</small></div><pre>{code.split('\n').map((line, index) => <div key={index}><span>{index + 1}</span><code>{line || ' '}</code></div>)}</pre></main></div>}
        </section>}
      </section>
    </main>
