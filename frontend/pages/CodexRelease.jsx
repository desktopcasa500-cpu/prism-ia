...
export default function CodexRelease() {
  ...
  const controllerRef = useRef(null); const bottomRef = useRef(null); const textareaRef = useRef(null);
    ...
    try {
      ...
      const result = await api.streamPost('/ai/generate/stream', {
        ...
        if (event.type === 'artifact' && event.path) {
          ...
          setSelectedPath(event.path); setTab('preview');
        }
  ...
  const previewHtml = useMemo(() => {
    ...
    if (!htmlFile) return '<!doctype html><html><body style="font-family:system-ui;padding:40px"><p>Nenhum HTML gerado.</p></body></html>';
    ...
    const js = files.filter((file) => /\.js$/i.test(file.path)).map((file) => String(file.content || '')).join('\n').replace(/<\/script/gi, '<\\/script');
    html = html.replace(/<\/head>/i, `${css ? `<style>${css}</style>` : ''}</head>`); return html.replace(/<\/body>/i, `${js ? `<script>${js}</script>` : ''}</body>`);
  }, [files]);
