...
export default function PrismCodexIntro({onComplete}){
 const [typed,setTyped]=useState(''); const [phase,setPhase]=useState('title');
 useEffect(()=>{
  const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
  if(reduced){localStorage.setItem(INTRO_KEY,'1');onComplete?.({prompt:PROMPT,model:'prism-taff-2.0'});return undefined;}
  const timers=[]; const later=(fn,ms)=>{const id=window.setTimeout(fn,ms);timers.push(id);};
  later(()=>setPhase('glitch'),5600);
  later(()=>setPhase('editorial'),9000);
  later(()=>setPhase('split'),16500);
  later(()=>setPhase('star'),23500);
  later(()=>setPhase('prism'),28500);
  later(()=>setPhase('codex'),34500);
  later(()=>setPhase('all'),40500);
  later(()=>setPhase('greeting'),45500);
  later(()=>setPhase('composer'),48500);
  later(()=>setPhase('models'),51500);
 ...
 useEffect(()=>{
  ...
  let index=0;setTyped('');const id=window.setInterval(()=>{index+=1;setTyped(PROMPT.slice(0,index));if(index>=PROMPT.length)window.clearInterval(id);},48);return()=>window.clearInterval(id);
 },[phase]);
