...
function preloadImages() { return Promise.all(SHOTS.map((shot) => new Promise((resolve) => { const image = new Image(); image.onload = resolve; image.onerror = resolve; image.src = shot.src; }))); }
export default function PrismCodexIntroEditV2({ onComplete }) {
  const [time, setTime] = useState(0); const [ready, setReady] = useState(false); const completedRef = useRef(false); const startRef = useRef(0); const rafRef = useRef(0);
  ...
  useEffect(() => { if (reduced) { complete(); return undefined; } let cancelled = false; preloadImages().finally(() => { if (cancelled) return; setReady(true); startRef.current = performance.now(); const tick = (now) => { if (cancelled) return; const elapsed = now - startRef.current; setTime(Math.min(elapsed, DURATION)); if (elapsed >= DURATION) { complete(); return; } rafRef.current = requestAnimationFrame(tick); }; rafRef.current = requestAnimationFrame(tick); }); const onKeyDown = (event) => { if (event.key === 'Escape') complete(); }; window.addEventListener('keydown', onKeyDown); return () => { cancelled = true; cancelAnimationFrame(rafRef.current); window.removeEventListener('keydown', onKeyDown); }; }, [reduced]);
  let sceneIndex = 0; SCENES.forEach((scene, index) => { if (time >= scene.at) sceneIndex = index; });
  const scene = SCENES[sceneIndex]; const nextScene = SCENES[Math.min(sceneIndex + 1, SCENES.length - 1)]; const shot = SHOTS[scene.shot]; const nextShot = SHOTS[nextScene.shot];
  const localProgress = Math.max(0, Math.min((time - scene.at) / scene.duration, 1)); const progress = `${Math.min((time / DURATION) * 100, 100)}%`; const transition = localProgress > .72 ? Math.min((localProgress - .72) / .28, 1) : 0;
  return <section className={`prism-edit-v2 ${ready ? 'is-ready' : 'is-loading'} edit-${scene.mode}`} style={{ '--scene-progress': localProgress, '--crossfade': transition }} aria-label="Apresentação do Prism Codex">
    <div className="pe2-bg pe2-bg-a" style={{ backgroundImage: `url(${shot.src})`, backgroundPosition: shot.position }} /><div className="pe2-bg pe2-bg-b" style={{ backgroundImage: `url(${nextShot.src})`, opacity: transition }} />
    ...
    <div className="pe2-chrome"><div>PRISM / CODEX</div><div><span>REAL MATERIAL</span><b>{String(sceneIndex + 1).padStart(2, '0')} / {String(SCENES.length).padStart(2, '0')}</b></div></div><div className="pe2-rule pe2-rule-top" /><div className="pe2-rule pe2-rule-bottom" style={{ '--progress': progress }}><i /></div>
    <div className="pe2-shot-meta">{shot.credit}<span />FILM / DIGITAL EDIT</div><div className="pe2-timecode">00:{String(Math.floor(time / 1000)).padStart(2, '0')}:{String(Math.floor((time % 1000) / 10)).padStart(2, '0')}</div><div className="pe2-center-mark" aria-hidden="true"><span>P</span><i /><i /></div>
